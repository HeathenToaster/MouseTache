...
def process_run_around_tower_epochs(all_epochs, time_video_frames, turns_df, distances, speeds):
    ...
    for run_index, run_around_tower in enumerate(runs_around_tower):
        ...
        is_maxNberOfConsecRewards = 'maxNberOfConsecRewards' in switch_in_turns_df.keys()

        if is_maxNberOfConsecRewards:
            
            maxNberOfConsecRewards = switch_in_turns_df.iloc[0]['maxNberOfConsecRewards']
            
        else:

            maxNberOfConsecRewards = None

        type_of_turn = {'Rewarded': '', 'direction': '', 'num_trapezeswitch': num_trapezeswitch, 'max_rewards': maxNberOfConsecRewards}
...
def calculate_time_distance_in_zones(all_trapezes_coordinates_cm, time_video_frames, smoothed_positions_cm, time_start=None, time_end=None):
    ...
    if time_start is None:
        time_start = time_video_frames[0]
    if time_end is None:
        time_end = time_video_frames[-1]

    # Find the indices corresponding to the specified time interval
    start_idx = np.searchsorted(time_video_frames, time_start)
    end_idx = np.searchsorted(time_video_frames, time_end)
    ...
    selected_time = time_video_frames[start_idx:end_idx]
...
def process_trajectory(folder_path_mouse_to_process,session_to_process,all_trapezes_coordinates_cm,towers_coordinates_cm):
    ...
    distances, speeds, angular_speeds = compute_distance_speed_angular_speed(smoothed_positions_cm,
                                        time_video_frames)
    ...
    # Calculate and print total distance in m
    ...
    all_epochs = define_epoch_types(clean_run_epochs, smoothed_positions_cm, time_video_frames,
                                    turns_df, all_trapezes_coordinates_cm)

    all_epochs = process_run_around_tower_epochs(all_epochs, time_video_frames,
                                                 turns_df, distances, speeds)

    run_around_tower_sessionresult = get_run_around_tower_resultssessions(all_epochs)

    all_epochs = process_other_epochs(all_epochs, time_video_frames, smoothed_positions_cm,
                                      all_trapezes_coordinates_cm, distances, speeds)

    time_spent_in_zones, distance_covered_in_zones = calculate_time_distance_in_zones(all_trapezes_coordinates_cm,
                                                    time_video_frames, smoothed_positions_cm,
    ...
    output_pickle_filepath = os.path.join(folder_path_mouse_to_process,
                                          session_to_process,output_pickle_filename)
    ...
    session_data = {
        ...
        'towers_coordinates_cm' : towers_coordinates_cm,
        'time_in_zones' : time_spent_in_zones,
        'distance_in_zones' : distance_covered_in_zones
    }
